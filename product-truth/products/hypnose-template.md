# RikaNV Hypnose — шаблон карточки product_line

Hypnose — `product_line`. Не использовать как конкретную модель.

Для specification claims требуется конкретная модель внутри линейки. Если модель не выбрана, все модельные характеристики остаются `N/A — product_line, model-level specs require concrete model.`

## Официальное название

RikaNV Hypnose

## Product Kind

product_line

## Product Category

observation_thermal

## Parent Line

N/A

## Краткое назначение

Line-level: линейка наблюдательных тепловизионных приборов. Формулировки для публикации требуют doctrine compliance review.

## Модели, найденные на rikanv.ru

| Модель | Источник | Статус |
|---|---|---|
| RikaNV Hypnose2 335L | src_rikanv_ru_hypnose2_335l_20260701 | candidate |
| RikaNV Hypnose2 650L | src_rikanv_ru_hypnose2_650l_20260701 | candidate |
| RikaNV Hypnose2 660D | src_rikanv_ru_hypnose2_660d_20260701 | candidate |

## Характеристики линейки

| Параметр | Значение | Источник | Статус |
|---|---|---|---|
| Сенсор | N/A — product_line, model-level specs require concrete model. | N/A | N/A |
| Объектив | N/A — product_line, model-level specs require concrete model. | N/A | N/A |
| NETD | N/A — product_line, model-level specs require concrete model. | N/A | N/A |
| Дисплей | N/A — product_line, model-level specs require concrete model. | N/A | N/A |
| Дальность | N/A — product_line, model-level specs require concrete model. | N/A | N/A |
| Видеозапись | Line-level mention requires model check before model-level use | src_rikanv_ru_hypnose_line_20260701 | draft |
| Конкретная модель | TODO: выбрать модель Hypnose/Hypnose2 | TODO | draft |
| Баллистический калькулятор | N/A — не относится к категории прибора | N/A | N/A |
| Пристрелка | N/A — не относится к категории прибора | N/A | N/A |

## Подтвержденные claims

| claim_id | Claim | Источник | Evidence | Status |
|---|---|---|---|---|
| claim-hypnose-line-001 | RikaNV Hypnose является продуктовой линейкой наблюдательных тепловизионных приборов. | src_rikanv_ru_hypnose_line_20260701 | ev_rikanv_ru_hypnose_line_20260701 | verified line-level |

## Ограничения

- Не описывать Hypnose как прицел.
- Не описывать Hypnose как конкретную модель.
- Не переносить line-level specs на конкретную модель без проверки product page.

## Дата последней проверки

2026-07-01
